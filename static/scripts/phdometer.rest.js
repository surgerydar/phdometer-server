//
// phdometer rest interface
//
;
(function() {
	phdometer.rest = phdometer.rest || {
		...
		get : function( url, delegate, headers ) {
			...
			// create request
			//
			var request = phdometer.rest.createrequest("GET", url, {}, delegate, headers);
			//
			// send
			// 
			request.send();
		...
		post : function( url, data, delegate, headers, raw ) {
		    ...
		    var request = phdometer.rest.createrequest("POST", url, {}, delegate, headers);
			//
			// send data
			// 
		    //
		...
		put : function( url, data, delegate, headers, raw ) {
		    ...
		    if (!headers) headers = {};
		    if (!headers['Content-Type']&&!raw) headers['Content-Type'] = 'application/json'; // default to JSON request body
		    var request = phdometer.rest.createrequest("PUT", url, {}, delegate, headers);
			//
			// send data
			// 
		    if ( raw ) {
		...
		delete : function( url, delegate, headers ) {
			...
			// create request
			//
			var request = phdometer.rest.createrequest("DELETE", url, {}, delegate, headers );
			//
			// send
			// 
			request.send();
